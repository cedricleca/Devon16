...
std::string TextEditor::GetText(const Coordinates & aStart, const Coordinates & aEnd) const
{
	...
	for (size_t i = lstart; i < unsigned(lend); i++)
		s += mLines[i].size();
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates & aFrom) const
{
	...
	while (!isword || skip)
	{
		if (unsigned(at.mLine) >= mLines.size())
		{
...
{
	if (unsigned(aCoordinates.mLine) >= mLines.size())
		return -1;
	auto& line = mLines[aCoordinates.mLine];
	int c = 0;
	unsigned int i = 0;
...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			for (int i = 0; unsigned(i) < line.size();)
			{
				auto& glyph = line[i];
				...
				if (glyph.mChar == '\t')
				{
					...
					if (mShowWhitespaces)
					{
						...
						drawList->AddLine(p1, p2, 0x70707070);
						drawList->AddLine(p2, p3, 0x70707070);
						drawList->AddLine(p2, p4, 0x70707070);
					}
				}
				else if (glyph.mChar == ' ')
				{
					if (mShowWhitespaces)
					{
						...
						const auto y = textScreenPos.y + bufferOffset.y + s * 0.5f;
						drawList->AddCircleFilled(ImVec2(x, y), 1.5f, 0x70707070, 4);
					}
...
void TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	...
	if (mLines.empty() || unsigned(oldPos.mLine) >= mLines.size())
		return;
	...
	while (aAmount-- > 0)
	{
		...
		if (unsigned(cindex) >= line.size())
		{
			if (unsigned(mState.mCursorPosition.mLine) < mLines.size() - 1)
			{
...
void TextEditor::Backspace()
{
	...
	else
	{
		...
		else
		{
			...
			while (unsigned(cindex) < line.size() && cend-- > cindex)
			{
				u.mRemoved += line[cindex].mChar;
				line.erase(line.begin() + cindex);
			}
...
void TextEditor::ColorizeInternal()
{
	...
	if (mCheckComments)
	{
		...
		auto currentLine = 0;
		auto currentIndex = 0;
		while (unsigned(currentLine) < endLine || currentIndex < endIndex)
		{
			...
			if (!line.empty())
			{
				...
				bool inComment = (commentStartLine < unsigned(currentLine) || (commentStartLine == currentLine && commentStartIndex <= currentIndex));
				...
				else
				{
					...
					else
					{
						...
						inComment = inComment = (commentStartLine < unsigned(currentLine) || (commentStartLine == currentLine && commentStartIndex <= currentIndex));
...
float TextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
{
	...
	float distance = 0.0f;
	float spaceSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ", nullptr, nullptr).x;
	unsigned int colIndex = GetCharacterIndex(aFrom);
...
const TextEditor::LanguageDefinition & TextEditor::LanguageDefinition::DevonASM()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
"org","entry","byte","word","long","define","include","incbin","romexport","align"
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("%[0-1]", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier));
...
bool TextEditor::SaveText(const std::string& Filename)
{
	if (IsReadOnly())
		return false;
