...
struct MemoryEditor
{
    ...
    u16             (*ReadFn)(u16* data, size_t off);        // = NULL   // optional handler to read bytes
    void            (*WriteFn)(u16* data, size_t off, u16 d); // = NULL   // optional handler to write bytes
    bool            (*HighlightFn)(u16* data, size_t off);   // = NULL   // optional handler to return Highlight property (to support non-contiguous highlighting)
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        s.HexCellWidth = (float)(int)(s.GlyphWidth * 5.0f);             // "FF " we include trailing space in the width to easily catch clicks everywhere
        ...
        if (OptShowAscii)
        {
            ...
            s.PosAsciiEnd = s.PosAsciiStart + Rows * s.GlyphWidth * 2;
        }
...
#define _PRISizeT   "zX"
    ...
    void DrawWindow(const char* title, u16* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
    ...
    void DrawContents(u16* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
        CalcSizes(s, mem_size, base_display_addr);
        ImGuiStyle& style = ImGui::GetStyle();
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
            {
                ...
                if (DataEditingAddr == addr)
                {
                    ...
                    struct UserData
                    {
                        ...
                        char   CurrentBufOverwrite[6];  // Input
                    ...
                    {
                        if (WriteFn)
                            WriteFn(mem_data, addr, (u16)data_input_value);
                        else
                            mem_data[addr] = (u16)data_input_value;
                    }
                    ImGui::PopID();
                }
                else
                {
                    ...
                    u16 b = (ReadFn ? ReadFn(mem_data, addr) : mem_data[addr]) & 0xFFFF;
            ...
            if (OptShowAscii)
            {
                ...
                {
                    DataEditingAddr = addr + (size_t)((ImGui::GetIO().MousePos.x - pos.x) / (s.GlyphWidth*2));
                    DataEditingTakeFocus = true;
                }
                ImGui::PopID();
                for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
                {
                    if (addr == DataEditingAddr)
                    {
                        ...
                        draw_list->AddRectFilled(pos, ImVec2(pos.x + s.GlyphWidth*2, pos.y + s.LineHeight), ImGui::GetColorU32(ImGuiCol_TextSelectedBg));
                    }
					u16 b = ReadFn ? ReadFn(mem_data, addr) : mem_data[addr];
					unsigned char c = b>>8;
