...
//		ImU32 id = window->GetID(label);
	...
	bool PathBox(const char* label, std::wstring& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			for (uint32_t i = 0; i < btnList.size(); i++) {
	...
	bool FavoriteButton(const char* label, bool isFavorite)
	{
		...
		float size = window->DC.ItemWidth;
	...
	void* FileDialog::m_getIcon(const std::wstring& path)
	{
		...
		for (uint32_t i = 0; i < modPath.size(); i++)
			if (modPath[i] == '/')
				modPath[i] = '\\';
	...
	{
		for (uint32_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
	...
	void FileDialog::m_renderContent()
	{
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_ScrollFreezeTopRow | */ImGuiTableFlags_Resizable | ImGuiTableFlags_Sortable, ImVec2(0, -1))) {
				...
				ImGui::TableSetupColumn("Date modified##filedate", ImGuiTableColumnFlags_WidthStretch/*ImGuiTableColumnFlags_WidthAlwaysAutoResize*/, -1, 1);
				ImGui::TableSetupColumn("Size##filesize", ImGuiTableColumnFlags_WidthStretch/*ImGuiTableColumnFlags_WidthAlwaysAutoResize*/, -1, 2);
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		ImGui::SameLine();
		if (ImGui::InputTextEx("##file_input", "Filename", m_inputTextbox, 1024, ImVec2((m_type != IFD_DIALOG_DIRECTORY) ? -250.f : -1.f, 0.f), ImGuiInputTextFlags_EnterReturnsTrue)) {
